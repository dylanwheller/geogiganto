...
const ASTRO_QUOTES = [
  ...
  'The world is too big to stay in one place and too beautiful to rush.',
...
export default function HomeScreen() {
  ...
  const loadDailyChallenge = useCallback((uid: string) => {
    const today = new Date().toISOString().slice(0, 10);
    AsyncStorage.getItem(`@geogiganto_daily_${uid}_${today}`)
      .then(val => {
        // Guard: discard result if the user changed while this request was in flight
        if (uidRef.current !== uid) return;
        if (val !== null) {
    ...
    <SafeAreaView className="flex-1 bg-background">
      <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingBottom: 160 }}>
        ...
        {continueTarget && !bossBattleWorld ? (
          <View className="px-6 mb-6">
            <Animated.View entering={FadeInDown.duration(600).delay(100)}>
              <CopilotStep
                name="home-continue"
                order={2}
                text="Tap here to jump straight into your next geography lesson."
              >
                <CopilotView>
                  <TouchableOpacity
                    onPress={() => {
          ...
          <CopilotStep name="home-continue" order={2} text="Tap here to jump straight into your next geography lesson.">
          ...
          <Animated.View entering={FadeInDown.duration(600).delay(300)}>
            <View className="bg-card rounded-2xl overflow-hidden border border-border">
              ...
              <View className="bg-primary/10 px-4 py-3 flex-row items-center gap-2">
                ...
                <Text className="text-primary font-bold">Daily Geography Challenge</Text>
              ...
              {dailyCard.done ? (
                ...
                >
                  <View className="p-4">
                    <Text className="text-foreground font-semibold mb-1">
                      10 geography questions · 60 seconds
                    </Text>
                    <Text className="text-muted-foreground text-sm mb-3">
                      Continents · Oceans · Climates · Flags & Capitals
                    </Text>
            ...
            <View className="bg-primary/5 rounded-2xl p-5 border border-primary/20">
              <View className="flex-row items-start gap-3">
                ...
                <View className="flex-1">
                  ...
                  <Text className="text-primary text-xs mt-2">— GeoGiganto Wisdom</Text>
                </View>
              </View>
            </View>
          </Animated.View>
