...
export default function LoginScreen() {
  ...
  if (isLoading) {
    return (
      <View className="flex-1 bg-[#0d2614] items-center justify-center">
      ...
      <View className="flex-1 px-6 pt-16 pb-8">
        ...
        <View className="items-center mb-8">
          <Image
            ...
            resizeMode="cover"
          />
          <Text className="text-3xl font-bold text-[#86efac] text-center">
            {mode === 'signin' ? 'Welcome, Explorer!' : 'Create account'}
          </Text>
          <Text className="mt-2 text-sm text-[#4ade80] text-center">
            {mode === 'signin'
              ? 'Sign in to continue your geography adventure.'
              : 'Join GeoGiganto and explore the world.'}
          </Text>
        </View>

        {/* Form card */}
        <View className="rounded-2xl bg-[#142e1c] p-6 gap-3 shadow-lg">

          {/* Google sign-in */}
          <TouchableOpacity
            onPress={handleGoogleSignIn}
            disabled={submitting}
            className="flex-row items-center justify-center gap-3 rounded-xl bg-[#1a3a23] border border-[#1a5229] px-4 py-3"
          >
          ...
          <View className="flex-row items-center gap-3 my-1">
            <View className="flex-1 h-px bg-[#1a5229]" />
            <Text className="text-xs text-[#4ade80]">or</Text>
            <View className="flex-1 h-px bg-[#1a5229]" />
          </View>
          ...
          {mode === 'signup' && (
            <TextInput
              ...
              className="rounded-xl border border-[#1a5229] bg-[#1a3a23] px-4 py-3 text-[#f0f9ff]"
          ...
          {/* Password */}
          <View className="flex-row items-center rounded-xl border border-[#1a5229] bg-[#1a3a23]">
            <TextInput
              value={password}
          ...
          <TouchableOpacity
            ...
            disabled={submitting}
            className="rounded-xl bg-[#4ade80] px-4 py-3 items-center mt-1"
          >
            {submitting ? (
          ...
          >
            <Text className="text-sm text-[#4ade80]">
              {mode === 'signin'
        ...
        <Text className="text-center text-xs text-[#4ade80]/60 mt-6">
          Version {version}
        </Text>
      </View>
    </KeyboardAvoidingView>
